...
export default function Chat(props) {
    ...
    return (
        <div className="w-full h-full max-sm:p-4 max-sm:pb-10">
            {!props.user && <div className={`w-full flex justify-end ${data.user.username != undefined && 'justify-between' } dark:bg-main_text border border-main_text px-4 py-3 h-20 rounded-xl`}>
                ...
                <div className="flex items-center">
                    <button className="w-10 h-10 p-1 flex justify-center items-center rounded-full border border-main_light_dark dark:hover:border-main_light_gray anim-500"><BsThreeDots className="text-xl dark:text-main_light" /></button>
                </div>
            </div>}
